...
const Pagination = ({
	...
	const renderPageButton = (p: number, label: string | number, isCurrent: boolean) => {
		return (
			<button
				key={p}
				className={`bg-light-gray hover:bg-light-green text-big-regular cursor-pointer rounded-full px-[14px] py-[5px] text-white ${
					isCurrent ? 'bg-light-green' : ''
				}`}
				disabled={isCurrent}
	...
	const renderEllipsis = (key: string) => (
		<span key={key} className='txt-xlarge-plus text-ui-fg-muted cursor-default items-center'>
			...
		</span>
	)
